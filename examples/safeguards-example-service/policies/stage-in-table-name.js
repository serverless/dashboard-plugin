    ...
    Object.entries(template.Resources).forEach(([name, resource]) => {
      if(resource.Type === 'AWS::DynamoDB::Table') {
        if(!resource.Properties.TableName.endsWith(`-${stage}`)) {
          throw new policy.Failure(`Expected TableName property of "${name}" to end with "-${stage}" to reflect the current stage. Instead found "${resource.Properties.TableName}".`)
        }
      }
    })
  }
