      ...
      if (resource.Type === 'AWS::DynamoDB::Table') {
        if (resource.Properties.ProvisionedThroughput.ReadCapacityUnits > limits.readCapacityMax) {
          throw new policy.Failure(
            `Table "${name}" has excess read capacity. Lower the value of ReadCapacityUnits to at most ${limits.readCapacityMax}.`
          );
        }
        ...
        if (
          ...
          throw new policy.Failure(
            `Table "${name}" has excess write capacity. Lower the value of WriteCapacityUnits to at most ${limits.writeCapacityMax}.`
          );
        }
      }
    });
