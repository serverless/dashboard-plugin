...
describe('outputCommand', function () {
  ...
  describe('list', () => {
    ...
    it('Should support `--org` `--app` CLI params', async () => {
      ...
      expect(stdOutput).to.include('stringOutputName');
      expect(stdOutput).to.include('stringOutputValue');
      expect(stdOutput).to.include('objectOutputName');
      expect(stdOutput).to.include('objectOutputPropName');
      expect(stdOutput).to.include('objectOutputPropValue');
      expect(stdOutput).to.include('arrayOutputName');
      expect(stdOutput).to.include('arrayOutputItem');
    });
    ...
    it('Should support `--stage` CLI param', async () => {
      ...
      expect(stdOutput).to.include('otherOutputName');
      expect(stdOutput).to.include('otherOutputName');
    });
    it('Should support `--region` CLI param', async () => {
      ...
      expect(stdOutput).to.include('usEast2OutputName');
      expect(stdOutput).to.include('usEast2OutputValue');
    });
  ...
  describe('get', () => {
    ...
    it('Should support `--org` and `--app` CLI params', async () => {
      ...
      expect(stdOutput).to.include('output-value');
    });
    it('Should support `--service` CLI params', async () => {
      let stdOutput = '';
      await overrideStdoutWrite(
