...
describe('logsCollection', () => {
  let getLogDestinationStub;
  let logsCollection;

  before(() => {
    getLogDestinationStub = sinon.stub().resolves({ destinationArn: 'arn:logdest' });
    logsCollection = proxyquire('./logs-collection', {
      './client-utils': {
        getPlatformClientWithAccessKey: () => ({
          logDestinations: {
            getOrCreate: getLogDestinationStub,
          },
        }),
      },
    });
  ...
  it('adds log subscription filter to template', async () => {
    ...
    expect(getLogDestinationStub.args[0][0]).to.deep.equal({
      appUid: 'app123',
      orgUid: 'org123',
      stageName: 'dev',
