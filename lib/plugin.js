...
class ServerlessEnterprisePlugin {
  constructor(sls) {
    ...
    const coreInteracivePlugin = sls.pluginManager.plugins.find(
      plugin => plugin.constructor.name === 'InteractiveCli'
    );
    if (coreInteracivePlugin) {
      if (!coreInteracivePlugin.commands.interactiveCli.options) {
        coreInteracivePlugin.commands.interactiveCli.options = {};
      }
      // Extend options
      coreInteracivePlugin.commands.interactiveCli.options.app = { usage: 'Dashboard app' };
      coreInteracivePlugin.commands.interactiveCli.options.org = { usage: 'Dashboard org' };
    }
  ...
  async asyncInit() {
    ...
    if (this.sls.interactiveCli && this.sls.interactiveCli.initializeProjectChoices) {
      for (let i = 0; i < this.sls.interactiveCli.initializeProjectChoices.length; i++) {
        if (
          !['aws-nodejs', 'aws-python3', 'aws-python'].includes(
            this.sls.interactiveCli.initializeProjectChoices[i].value
          )
        ) {
          this.sls.interactiveCli.initializeProjectChoices.splice(i, 1);
          i--;
        }
      }
    }
