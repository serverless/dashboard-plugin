    ...
    return async () => {
      ...
      switch (hook) {
        ...
        case 'before:step-functions-offline:start':
          ...
          break;
        case 'login:login': {
          const isConsole = this.sls.configurationInput && this.sls.configurationInput.console;
          await login({ isConsole });
          if ((!this.sls.service.org || !this.sls.service.app) && !isConsole) {
            log.notice();
            log.notice(
              'Run "serverless" to integrate your Service with Serverless Framework Dashboard'
            );
          }
          if (isConsole) {
            log.notice();
