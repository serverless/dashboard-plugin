...
class ServerlessEnterprisePlugin {
  constructor(sls) {
    ...
    // maintaining backward compatibility with old "serverless" versions which do not expose it
    // TODO: With next major assume "sls._commandsSchema" unconditionally
    const commandsSchema = sls._commandsSchema || new Map();
      ...
      switch (hook) {
        ...
        case 'login:login':
          ...
          if (!this.sls.service.org || this.sls.service.app) {
            this.sls.cli.log("Please run 'serverless' to configure your service");
          }
          break;
        case 'logout:logout':
          await logout(this);
    ...
    if (this.sls.interactiveCli && this.sls.interactiveCli.initializeServiceConfiguration) {
      ...
      if (this.sls.processedInput.options.org || this.sls.processedInput.options.app) {
        this.sls.interactiveCli.initializeServiceConfiguration.initializeProjectChoices =
          this.sls.interactiveCli.initializeServiceConfiguration.initializeProjectChoices.filter(
