...
class ServerlessEnterprisePlugin {
  constructor(sls) {
    ...
    if (sls.allowedInteractiveCliOptions) {
      sls.allowedInteractiveCliOptions.add('app');
      sls.allowedInteractiveCliOptions.add('org');
    }
    // Add interactive CLI hooks
    this.hooks = { ...this.hooks, ...interactiveCli(this) };
  ...
  route(hook) {
    ...
    return async () => {
      ...
      const currentCommand = self.sls.processedInput.commands[0];
      ...
      switch (hook) {
        ...
        case 'before:deploy:deploy':
          self.enterprise = {
            errorHandler: errorHandler(self), // V.1 calls this when it crashes
          };
