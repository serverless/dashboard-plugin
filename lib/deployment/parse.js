...
const parseDeploymentData = async (ctx, status = 'success', error = null, archived = false) => {
  ...
  if (!archived) {
    ...
    if (vcs.type === 'git') {
      ...
      vcs.commit = await (async () => {
        try {
          return (await git.raw(['show', '-s', '--format=%H', branch.current])).trim();
        } catch (gitShowError) {
          if (gitShowError.message.includes('fatal:')) {
      ...
      if (vcs.commit) {
        vcs.commitMessage = (await git.raw(['show', '-s', '--format=%B', branch.current])).trim();
        vcs.committerEmail = (await git.raw(['show', '-s', '--format=%ae', branch.current])).trim();
      }
