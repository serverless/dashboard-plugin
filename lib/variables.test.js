...
const apiRequestPath = require.resolve('@serverless/utils/api-request');
const resolveAuthModePath = require.resolve('@serverless/utils/auth/resolve-mode');
...
const modulesCacheStub = {
  ...
  [resolveAuthModePath]: async () => 'user',
  [apiRequestPath]: async (requestPath, options = {}) => {
    if (requestPath.startsWith('/api/identity/orgs/name')) {
      return { orgId: 'consoleOrgId' };
    }
    if (requestPath.startsWith('/api/secrets/')) {
      const secretsPath = options.body[0].path;
      const serviceName = secretsPath.split('/')[0];
      return {
        secrets: [
          {
            name: 'stageLocal',
            path: secretsPath,
            parameter: { value: 'unexpectedDashboardInstance' },
          },
          {
            name: 'consoleInstance',
            path: secretsPath,
            parameter: { value: 'consoleInstanceValue' },
          },
          {
            name: 'defaultLocal',
            path: serviceName,
            parameter: { value: 'unexpectedDashboardService' },
          },
          {
            name: 'consoleService',
            path: serviceName,
            parameter: { value: 'consoleServiceValue' },
          },
        ],
      };
    }
...
describe('lib/variables.test.js', () => {
    ...
    ({
      ...
      configExt: {
        custom: {
          ...
          paramConsoleInstance: '${param:consoleInstance}',
          paramDashboardInstance: '${param:dashboardInstance}',
          paramDefaultLocal: '${param:defaultLocal}',
          paramConsoleService: '${param:consoleService}',
        ...
        params: {
          default: {
            ...
            consoleInstance: 'unexpectedDefaultLocal',
  ...
  it('should resolve param source', async () => {
    ...
    expect(configurationInput.custom.paramConsoleInstance).to.equal('consoleInstanceValue');
    ...
    expect(configurationInput.custom.paramConsoleService).to.equal('consoleServiceValue');
