...
const requireDlq = sinon.stub().callsFake(policy => {
  policy.approve();
});
const iamPolicy = sinon.stub().callsFake(policy => {
  policy.approve();
});
const secretsPolicy = sinon.stub().callsFake(policy => {
  policy.fail('Error Message');
  policy.fail('Error Message');
});

const runPolicies = proxyquire('./', {
