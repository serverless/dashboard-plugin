...
module.exports.configureDeployProfile = async (ctx) => {
  ...
  let useProviders = true;
  for (const { name, value } of Object.values([
    { name: 'app', value: app },
    { name: 'org', value: org },
    ...
    if (value.match(/(\${.*\}|!.*)/)) {
      ...
      ctx.sls._logDeprecation(
        `PARAMETERIZED_ARGUMENT_${name.toUpperCase()}`,
        `Cannot parameterize "${name}" with "${value}", please use plaintext or pass arguments via command line options`
      );
    }
  }
  try {
