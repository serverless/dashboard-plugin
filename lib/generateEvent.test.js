...
describe('generating events', () => {
  it('builds http events', async () => {
    ...
    expect(await generate.bind(that)(ctx)).to.deep.equal({
      body: '{"foo": "bar"}',
      path: '/test/hello',
      headers: {
    ...
    const ctx = {
      sls: {
        processedInput: {
          options: {
            type: 'aws:dynamo',
            body: '{ "Keys": { "Id": { "N": "101" } }, "NewImage": { "Message": { "S": "New item!" }, "Id": { "N": "101"}',
          },
        },
      },
    };
  ...
  it('throws errors for invalid events', async () => {
    ...
    try {
      ...
      expect(error.message).to.include('Invalid event specified');
    }
  });
});
