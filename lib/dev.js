...
module.exports = async function(ctx) {
  ...
  /**
   * For now, you can only run this command in the same folder as your serverless.yml
   */
  if (!existsSync('./serverless.yml')) {
    throw new ServerlessError(
      "This command must be run in the same directory as your 'serverless.yml'.",
      'DEV_BAD_FOLDER'
    );
  }

  if (!isAuthenticated()) {
    throwAuthError(ctx.sls);
  }
