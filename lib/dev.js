...
module.exports = async function(ctx) {
  ...
  const publishAppState = async () => {
    ...
    return { watchedFilenameToFunctionName, functionHandlers, output };
  };

  /**
   * Push initial serverless.yml state to the socket
   */
  ...
  const rewatchfiles = async () => {
    try {
       ...
       * Compute new watch files. Only rewatch new files if parsing the serverless.yml file is successfull.
       */
      const { watchedFilenameToFunctionName, functionHandlers } = publishAppState();
