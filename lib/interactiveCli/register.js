...
const generateTenantName = email => {
  ...
  return keepValidTenantNameChars(userName);
};
...
const signUp = async (inquirer, email = null) => {
  ...
  const userName = generateUserName(email);
  const orgName = generateTenantName(email);
...
const steps = {
  registerOrLogin: async serverless => {
    ...
    const { ownerUserName, tenantName: orgName, ownerAccessKey, ownerAuth0Id } = await signUp(
      inquirer
    );
