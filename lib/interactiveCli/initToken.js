...
const { readConfigFile, writeConfigFile } = require('../configFile');
...
const { appNameInput } = require('./appNameInput');
...
module.exports = {
  ...
  async run(serverless) {
    ...
    if (template.url && template.directory) {
      ...
      // TODO refactor out to template
      await spawn('npm', ['install']);
      await serverless.reInitialize();
      const apps = await sdk.listApps(tenantName);
      const appName = await appNameInput(
        inquirer,
        apps.map((app) => app.appName)
      );
      await writeOrgAndApp(serverless, tenantName, appName);
      await createAppWithDeploymentProfile(inquirer, tenantName, secretAccessKey, appName);
      process.stdout.write(
        `\n${chalk.green(`Project successfully created in '${template.directory}' folder.`)}\n`
      );
    }
