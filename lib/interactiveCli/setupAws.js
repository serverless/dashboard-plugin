...
const validManagedAccountStatuses = new Set(['IN_PROGRESS', STATUS_IN_PROGRESS, STATUS_EXPIRED]);
  ...
  steps.shouldSetupAwsCredentials = async () => {
    ...
    switch (registrationType) {
      case 'preview': {
        ...
        let awsPreviewManagedAccount = await getAwsPreviewManagedAccount({
          accessKey: await resolveAccessKey(user, tenantName),
          tenantName,
        });
        user = getLoggedInUser(); // Token might have been refreshed
        ...
        while (awsPreviewManagedAccount.status === STATUS_IN_PROGRESS) {
          await sleep(2000);
          awsPreviewManagedAccount = await getAwsPreviewManagedAccount({
            accessKey: await resolveAccessKey(user, tenantName),
            tenantName,
          });
        }
