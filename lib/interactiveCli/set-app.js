...
const steps = {
  ...
  setTenantAndApp: async (
    ...
    if (
      ...
      const { writeConfig } = await inquirer.prompt({
...
module.exports = {
  async check(serverless) {
    ...
    if (tenantName && tenantNames.has(tenantName)) {
      ...
      if (serverless.processedInput.options.org || serverless.processedInput.options.app) {
        ...
        serverless.cli.log(`Sorry, the app ${appName} is not available in the org ${tenantName}.`);
  ...
  async run(serverless, options) {
    ...
    if (!options.tenantName) {
      ...
      } else if (!(await steps.enableConfirm(inquirer))) {
        return;
      }
      ...
      if (
        ...
        !(await steps.enableConfirm(inquirer))
      ) {
        return;
      }
