...
const { getPlatformClientWithCredentials, getOrCreateAccessKeyForOrg } = require('../clientUtils');
...
const createAppWithDeploymentProfile = async (sdk, inquirer, orgName, accessKey, newAppName) => {
...
const steps = {
  ...
  setOrgAndApp: async (serverless, { user, orgNames, orgName, apps, appName, newAppName }) => {
    ...
    if (newAppName) {
      appName = await createAppWithDeploymentProfile(sdk, inquirer, orgName, accessKey, newAppName);
    }
    if (
