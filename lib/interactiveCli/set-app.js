...
const steps = {
  ...
  resolveOrgNames: async user => {
    ...
    if (!user.idToken) {
      ...
      // Handling it gently by assuming that orgss listed in config file
  ...
  setOrgAndApp: async (serverless, { user, orgNames, orgName, apps, appName, newAppName }) => {
    ...
    if (!orgName) {
      orgName = await (async () => {
        ...
        if (serverless.service.org && orgNames.has(serverless.service.org)) {
          return serverless.service.org;
        }
    ...
    if (
      ...
      serverless.service.org &&
      (orgName !== serverless.service.org || appName !== serverless.service.app)
    ) {
      const { shouldOverrideDashboardConfig } = await inquirer.prompt({
        message: `Are you sure you want to update monitoring settings from ${chalk.bold(
          `app: ${serverless.service.app}, org: ${serverless.service.org}`
...
module.exports = {
  async check(serverless) {
    ...
    const orgName = serverless.processedInput.options.org || serverless.service.org;
