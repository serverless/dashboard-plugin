...
module.exports = ctx => {
  ...
  const registerAndSetApp = async () => {
    ...
    if (ctx.sls.service.app && ctx.sls.service.tenant && user) {
      ...
      await configureDeployProfile({ ...ctx, provider: ctx.sls.getProvider('aws') });
    }
  };
...
`);
  ...
  if (user) hooks['after:interactiveCli:setupAws'] = showDoneMsg;
  else hooks[hook] = hooks[hook].then(showDoneMsg);
