...
const steps = {
  registerOrLogin: async (context) => {
    const registerOrLogin = await registerQuestion(context.inquirer);
    if (registerOrLogin === 'login') {
      await login();
      return;
    }
    const { ownerUserName, orgName, ownerAccessKey, ownerUserUid } = await signUp(context.inquirer);
    ...
    const { appName } = await sdk.apps.create({
      orgName,
      app: { name: `${context.configuration.service.name || context.configuration.service}-app` },
    });
...
module.exports = {
  ...
  async run(context) {
    if (!(await enableConfirm(context.inquirer, context.options))) {
      return null;
    }
    process.stdout.write('You are not logged in or you do not have a Serverless account.\n\n');
    return steps.registerOrLogin(context);
  },
  steps,
};
