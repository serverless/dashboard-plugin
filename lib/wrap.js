  ...
  return new Promise((resolve, reject) => {
    serverlessSDK
      ...
      .then(() => {
        try {
          const userHandler = require('./${fn.entryOrig}.js');
          const wrappedHandler = serverlessSDK.handler(userHandler.${fn.handlerOrig}, handlerWrapperArgs);
          const result = wrappedHandler(event, context, callback);        
          // if a promise was returned, we need to return a promise
          if (result && typeof result.then === 'function') {
            result.then((handlerReturn) => resolve(handlerReturn))
          } else {
            resolve(result)
          }
        } catch (error) {
      ...
      .catch((error) => {
        console.error('Unable to start dev mode', error);
        reject(error);
      })    
  });
};`;
