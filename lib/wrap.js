...
const nodeRuntimeSupportingESModules = (runtime) =>
  runtime && supportedNodeRuntime(runtime) && new Set(['nodejs14.x', 'nodejs16.x']).has(runtime);
...
const wrapNodeJs = (fn, ctx, accessKey) => {
  // In Node 12, we want to keep 'js', but use 'cjs' in Node 14 and higher to properly enforce CommonJS
  // even if user wants to use ES Modules for their code
  let newModulesExtension = 'js';
  let standardHandlerExec;
  if (fn.runtime === 'nodejs16.x') {
    newModulesExtension = 'cjs';
    // In this case, we can take advantage of the fact that Node 16 supports async handler resolution
    standardHandlerExec = `
try {
  try {
    ...
    if (e.code === 'ERR_REQUIRE_ESM') {
      const eventualUserHandler = (async () => {
        return (await import('./${fn.entryOrig}.js'));
      })();
      module.exports = (async () => {
        const userHandler = await eventualUserHandler;
        return {
          handler: serverlessSDK.handler(userHandler.${fn.handlerOrig}, handlerWrapperArgs)
        };
      })();
    } else {
      throw e;
  ...
  } else if (fn.runtime === 'nodejs14.x') {
    newModulesExtension = 'cjs';
    // In this case, we can take advantage of the fact that Node 14 has `deasyncify` utility
    // available as a part of runtime
    standardHandlerExec = `
try {
  try {
    const userHandler = require('./${fn.entryOrig}.js');
  ...
  module.exports.handler = serverlessSDK.handler(() => { throw error }, handlerWrapperArgs);
    ...
    // Leave it as-is for nodejs12.x runtime
    standardHandlerExec = `
try {
  const userHandler = require('./${fn.entryOrig}.js');
  module.exports.handler = serverlessSDK.handler(userHandler.${fn.handlerOrig}, handlerWrapperArgs);
...
var serverlessSDK = require('./serverless_sdk/index.${newModulesExtension}');
...
${isDevMode ? devModeHandlerExec : standardHandlerExec}`;
  // Create new handlers
  fs.writeFileSync(
    path.join(ctx.sls.config.servicePath, `${fn.entryNew}.${newModulesExtension}`),
    newHandlerCode
  );
};
...
const wrap = async (ctx) => {
  ...
  if (vals.some(({ runtime }) => nodeRuntimeSupportingESModules(runtime))) {
    const pathSdk = path.resolve(__dirname, '../sdk-js/dist/index.js');
    const pathSdkDestCJS = path.join(ctx.state.pathAssets, './index.cjs');
    fs.copySync(pathSdk, pathSdkDestCJS);
  }
  if (
    vals.some(
      ({ runtime }) => supportedNodeRuntime(runtime) && !nodeRuntimeSupportingESModules(runtime)
    )
  ) {
    const pathSdk = path.resolve(__dirname, '../sdk-js/dist/index.js');
  ...
  // add include directives for handler file & sdk lib
    ...
    let extension = 'cjs';
    if (ctx.sls.service.provider.runtime === 'nodejs12.x') {
      extension = 'js';
    }
