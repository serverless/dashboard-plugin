  ...
  Records: [
    {
      ...
      awsRegion: 'us-west-2',
      eventName: 'INSERT',
      eventSourceARN: 'arn:aws:dynamodb:us-east-1:123456789012:my-table-stream',
...
describe('eventDetection', () => {
  ...
  it('identifies dynamodb streams', () => {
    expect(detectEventType(dynamodb)).to.equal('aws.dynamodb');
  });

  it('identifies cloudwatch logs', () => {
    expect(detectEventType(cloudwatchLog)).to.equal('aws.cloudwatch.log');
  });

  it('identifies cloudwatch events', () => {
    expect(detectEventType(cloudwatchEvent)).to.equal('aws.cloudwatch.event');
  });

  it('identifies ses', () => {
    expect(detectEventType(ses)).to.equal('aws.ses');
  });
