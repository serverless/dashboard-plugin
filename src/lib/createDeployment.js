...
export default async function (ctx) {
  ...
  const serverlessStateFilePath = path.join(
    ctx.sls.config.servicePath, '.serverless', 'serverless-state.json'
  )
  ...
  if (!serverlessStateFile) {
    throw new Error(`Serverless Enterprise: Unable to save deployment due to missing "serverless-state.json"`)
  }
